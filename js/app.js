...
(function (global) {
     ...
     * and strictly rejects wrong/invalid UIDs!
     */
    async function verifyPlayerId() {
        ...
        statusBox.innerHTML = `
            <div class="flex items-center gap-2 text-yellow-400">
                ...
                <span>Querying Garena Free Fire servers for UID ${Security.escapeHTML(uid)} (${region})...</span>
            </div>`;
        ...
        // Try active lookup endpoints with CORS proxies
        const lookupURLs = [
            `https://free-ff-api-with-region.vercel.app/api/v1/account?region=${encodeURIComponent(region)}&uid=${encodeURIComponent(uid)}`,
            `https://api.allorigins.win/raw?url=${encodeURIComponent(`https://freefireinfo-zy9l.onrender.com/api/v1/account?region=${region}&uid=${uid}`)}`,
        ...
        if (fetchedName && fetchedName.trim().length > 0 && !fetchedName.toLowerCase().includes('not found') && !fetchedName.toLowerCase().includes('error')) {
            ...
            // FAILED or INVALID: NEVER MARK AS VERIFIED!
            state.verifiedInfo.verified = false;
            state.verifiedInfo.nickname = '';
            nameInput.value = '';
            ...
            statusBox.innerHTML = `
                <div class="flex items-center gap-2 text-red-400">
                    ...
                    <span><b>Player Not Found!</b> No active Free Fire account found for UID: <b>${Security.escapeHTML(uid)}</b> in region <b>${Security.escapeHTML(region)}</b>.</span>
                </div>
