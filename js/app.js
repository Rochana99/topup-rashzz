 ...
 * Real Garena Free Fire Player UID & Region Verification
 */
...
(function (global) {
    ...
    const CONFIG = Object.freeze({
        ...
        // Live Cloudflare Worker URL for real-time Garena Free Fire Verification
        VERIFY_WORKER_URL: 'https://jolly-sun-32a3.ffismylife6.workers.dev'
    });
    ...
    function sanitizeAndUpdateUID() {
        ...
        state.verifiedInfo.isManual = false;

        const nameInput = document.getElementById('playerName');
        if (nameInput) {
            nameInput.value = '';
            nameInput.readOnly = true;
            nameInput.classList.remove('bg-brand-dark', 'border-brand-red');
            nameInput.classList.add('bg-brand-dark/50', 'cursor-not-allowed');
        }

        const regionBadge = document.getElementById('regionBadge');
        if (regionBadge) regionBadge.classList.add('hidden');

        updateSummary();
    }

    /**
     * Handle Server Region change
     */
    function onRegionChange() {
        const regionSelect = document.getElementById('playerRegion');
        if (!regionSelect) return;
        state.verifiedInfo.region = regionSelect.value;
        const badge = document.getElementById('regionBadge');
        if (badge) badge.innerText = `[${regionSelect.value}]`;
        
        // Invalidate previous verification on region switch
        state.verifiedInfo.verified = false;
        state.verifiedInfo.nickname = '';
        const nameInput = document.getElementById('playerName');
    ...
    /**
     * Enable manual nickname editing
     */
    function enableManualNickname() {
        const nameInput = document.getElementById('playerName');
        const statusBox = document.getElementById('idStatus');
        const uidInput = document.getElementById('playerId');
        const regionSelect = document.getElementById('playerRegion');

        const uid = uidInput ? Security.sanitizeUID(uidInput.value) : '';
        const region = regionSelect ? regionSelect.value : 'IND';
        ...
        if (nameInput) {
            ...
            nameInput.classList.remove('cursor-not-allowed', 'bg-brand-dark/50');
            nameInput.classList.add('bg-brand-dark', 'border-brand-red', 'focus:ring-1', 'focus:ring-brand-red');
            nameInput.placeholder = 'Enter in-game nickname';
            nameInput.focus();
            nameInput.oninput = function () {
                const val = nameInput.value.trim();
                if (val.length >= 2) {
                    state.verifiedInfo.uid = uid;
                    state.verifiedInfo.nickname = val;
                    state.verifiedInfo.region = region;
                    state.verifiedInfo.verified = true;
                    state.verifiedInfo.isManual = true;
                } else {
                    state.verifiedInfo.verified = false;
                }
                updateSummary();
        ...
        if (statusBox) {
            statusBox.className = 'mt-4 p-3.5 rounded-xl bg-yellow-950/30 border border-yellow-500/50 text-xs flex items-center justify-between transition-all';
            statusBox.innerHTML = `
                <div class="flex items-center gap-2 text-yellow-300">
                    <i class="fa-solid fa-pen-to-square text-yellow-400 text-sm"></i>
                    <span>Type your exact Free Fire nickname above. Admin will verify your UID (${uid}) in-game before delivery.</span>
                </div>
            `;
        }
    }
    ...
    /**
     * Real Player UID Verification
     * Queries live Free Fire endpoints, extracts real IGN and Region.
     * Distinguishes between API outage vs explicit "player not found".
     */
    async function verifyPlayerId() {
        ...
        // 1. Validation check on digits
        if (!uid || uid.length < 5 || uid.length > 11) {
            showToast('Please enter a valid Free Fire UID (5-11 digits)', true);
            statusBox.className = 'mt-4 p-3.5 rounded-xl bg-red-950/40 border border-red-500/60 text-xs flex items-center justify-between transition-all';
            statusBox.innerHTML = `
                <div class="flex items-center gap-2 text-red-400">
                    <i class="fa-solid fa-triangle-exclamation text-brand-red text-sm"></i>
                    <span>Invalid UID length. Free Fire UIDs must be between 5 to 11 numeric digits.</span>
                </div>`;
            state.verifiedInfo.verified = false;
            nameInput.value = '';
            updateSummary();
        ...
        // 2. Loading Indicator — show auto-region scanning message
        statusBox.className = 'mt-4 p-3.5 rounded-xl bg-brand-dark border border-brand-border text-xs flex items-center justify-between transition-all';
        statusBox.innerHTML = `
            <div class="flex items-center gap-2 text-yellow-400">
                <i class="fa-solid fa-spinner fa-spin text-sm"></i>
                <span>Querying Garena Free Fire servers for UID <b class="text-white">${Security.escapeHTML(uid)}</b>... Checking live in-game status.</span>
            </div>`;

        let fetchedName = '';
        let fetchedRegion = region;
        let fetchedLevel = null;
        let apiReachable = false;        // At least one API gave a real HTTP response
        let apiExplicitNotFound = false; // API explicitly said this player doesn't exist

        // Try Live Cloudflare Worker Proxy (Redxgame Live Engine)
        if (CONFIG.VERIFY_WORKER_URL) {
            try {
                const wController = new AbortController();
                const wTimeout = setTimeout(() => wController.abort(), 6000);
                const workerRes = await fetch(`${CONFIG.VERIFY_WORKER_URL}?uid=${encodeURIComponent(uid)}`, {
                ...
                if (workerRes && workerRes.ok) {
                    apiReachable = true;
                    const wData = await workerRes.json().catch(() => null);
                    if (wData && wData.status === true && wData.nickname) {
                        fetchedName = wData.nickname;
                        const rawReg = (wData.region || '').toLowerCase();
                        if (rawReg.includes('singapore') || rawReg === 'sg') fetchedRegion = 'SG';
                        else if (rawReg.includes('india') || rawReg === 'ind') fetchedRegion = 'IND';
                        else if (rawReg.includes('bangladesh') || rawReg === 'bd') fetchedRegion = 'BD';
                        else if (rawReg.includes('pakistan') || rawReg === 'pk') fetchedRegion = 'PK';
                        else if (rawReg.includes('middle') || rawReg === 'me') fetchedRegion = 'ME';
                        else fetchedRegion = region;

                        if (regionSelect) {
                            regionSelect.value = fetchedRegion;
                            state.verifiedInfo.region = fetchedRegion;
                        }
                    } else if (wData && wData.status === false) {
                        apiExplicitNotFound = true;
                    }
                }
            } catch (_) {}
        }

        // All possible regions to auto-scan if not already verified
        const ALL_REGIONS = ['IND', 'SG', 'BD', 'PK', 'ME', 'NA', 'SAC', 'GLOBAL'];
        const regionsToTry = [region, ...ALL_REGIONS.filter(r => r !== region)];

        // Helper: try one endpoint URL and parse result
        async function tryFetch(url) {
            try {
                ...
                if (resp && resp.ok) {
                    ...
                    if (!data) return null;
                    if (data.error || data.message === 'Not Found' || data.success === false) {
                        apiExplicitNotFound = true;
                        return null;
                    }
                    if (data.basicInfo && (data.basicInfo.nickname || data.basicInfo.Name)) {
                        return {
                            name: data.basicInfo.nickname || data.basicInfo.Name,
                            region: data.basicInfo.region || null,
                            level: data.basicInfo.level || null
                        };
                    }
                    if (data.nickname || data.Name || data.player_name) {
                        return {
                            name: data.nickname || data.Name || data.player_name,
        ...
        // Scan regions: selected first, then others, stop on first hit
        for (const tryRegion of regionsToTry) {
            if (fetchedName || apiExplicitNotFound) break;
            const results = await Promise.any([
                tryFetch(`https://free-ff-api-with-region.vercel.app/api/v1/account?region=${encodeURIComponent(tryRegion)}&uid=${encodeURIComponent(uid)}`),
                tryFetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(`https://freefireinfo-zy9l.onrender.com/api/v1/account?region=${tryRegion}&uid=${uid}`)}`)
            ].map(p => p.then(r => r ? r : Promise.reject()))).catch(() => null);

            if (results && results.name) {
                fetchedName = results.name;
                fetchedRegion = results.region || tryRegion;
                fetchedLevel = results.level;
                // Auto-update the region dropdown to match found region
                if (regionSelect) {
                    regionSelect.value = fetchedRegion;
                    state.verifiedInfo.region = fetchedRegion;
                }
        ...
        // Also try region-independent endpoints as fallback
        if (!fetchedName && !apiExplicitNotFound) {
            const fallbackUrls = [
                `https://api.allorigins.win/raw?url=${encodeURIComponent(`https://freefireapi.com.br/api/search_id?id=${uid}`)}`,
                `https://freefireapi.me/api/info?uid=${encodeURIComponent(uid)}`
            ];
            for (const url of fallbackUrls) {
                if (fetchedName) break;
                try {
                    const controller = new AbortController();
                    ...
                    if (response && response.ok) {
                        ...
                        if (data) {
                            ...
                            if (data.basicInfo && (data.basicInfo.nickname || data.basicInfo.Name)) {
                                fetchedName = data.basicInfo.nickname || data.basicInfo.Name;
                                fetchedRegion = data.basicInfo.region || region;
                                fetchedLevel = data.basicInfo.level || null;
                                break;
                            }
                            if (data.nickname || data.Name || data.player_name) {
                                fetchedName = data.nickname || data.Name || data.player_name;
                                fetchedRegion = data.region || region;
                                fetchedLevel = data.level || null;
                                break;
                            }
                        }
                    }
        ...
        // 3. Three-way evaluation of Result
        if (fetchedName && fetchedName.trim().length > 0 && !fetchedName.toLowerCase().includes('not found') && !fetchedName.toLowerCase().includes('error')) {
            // ✅ API returned real player data
            const cleanName = Security.escapeHTML(fetchedName.trim());
            ...
            if (regionBadge) {
                regionBadge.innerText = `[${fetchedRegion}]`;
                regionBadge.classList.remove('hidden');
            }

            statusBox.className = 'mt-4 p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/60 text-xs flex items-center justify-between transition-all';
            statusBox.innerHTML = `
                <div class="flex items-center gap-2 text-emerald-400">
                    <i class="fa-solid fa-circle-check text-base"></i>
                    <span>Player Verified: <b class="text-white">${cleanName}</b> | Region: <b class="text-yellow-400">${fetchedRegion}</b> ${fetchedLevel ? `| Level: <b>${fetchedLevel}</b>` : ''}</span>
                </div>`;

            showToast(`Player Verified: ${cleanName} (${fetchedRegion})`);
            updateSummary();

        } else if (apiReachable && apiExplicitNotFound) {
            // ❌ API was reachable and explicitly confirmed this UID doesn't exist
            state.verifiedInfo.verified = false;
            state.verifiedInfo.nickname = '';
            nameInput.value = '';
            ...
            statusBox.className = 'mt-4 p-3.5 rounded-xl bg-red-950/40 border border-red-500/60 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2 transition-all';
            statusBox.innerHTML = `
                <div class="flex items-center gap-2 text-red-400">
                    <i class="fa-solid fa-circle-xmark text-brand-red text-base"></i>
                    <span><b>Player Not Found!</b> No active Free Fire account for UID: <b>${Security.escapeHTML(uid)}</b> in region <b>${Security.escapeHTML(region)}</b>. Please check your UID and region.</span>
                </div>
                <button type="button" onclick="App.enableManualNickname()" class="px-2.5 py-1 bg-brand-dark hover:bg-black text-yellow-400 hover:text-white border border-yellow-500/40 rounded-lg text-[11px] font-bold tracking-wide transition-colors whitespace-nowrap self-end sm:self-auto">
                    <i class="fa-solid fa-pen-to-square mr-1"></i> Enter Name Manually
                </button>`;

            showToast('Player UID not found! Please check your ID and Region.', true);
            updateSummary();

        } else {
            // ⚠️ ALL APIs timed out / unreachable — this is an API outage, NOT the player's fault
            // Auto-enable manual nickname so real players can still place orders
            state.verifiedInfo.uid = uid;
            state.verifiedInfo.region = region;
            state.verifiedInfo.verified = false;

            if (regionBadge) {
                regionBadge.innerText = `[${region}]`;
                regionBadge.classList.remove('hidden');
            }

            // Auto-unlock name input
            nameInput.readOnly = false;
            nameInput.classList.remove('cursor-not-allowed', 'bg-brand-dark/50');
            nameInput.classList.add('bg-brand-dark', 'border-brand-red');
            nameInput.placeholder = 'Type your exact in-game nickname';
            nameInput.focus();
            nameInput.oninput = function () {
                const val = nameInput.value.trim();
                if (val.length >= 2) {
            ...
            statusBox.className = 'mt-4 p-3.5 rounded-xl bg-yellow-950/30 border border-yellow-500/50 text-xs flex items-start gap-2.5 transition-all';
            statusBox.innerHTML = `
                <i class="fa-solid fa-wifi text-yellow-400 text-sm mt-0.5 flex-shrink-0"></i>
                <span class="text-yellow-300 leading-relaxed">
                    <b class="text-yellow-200">Garena lookup servers temporarily unreachable.</b><br>
                    UID <b class="text-white">${Security.escapeHTML(uid)}</b> [${Security.escapeHTML(region)}] accepted.
                    Please type your <b class="text-white">exact in-game nickname</b> in the field above — our admin will verify your account before delivery. ✅
                </span>`;

            showToast('Please type your in-game nickname to continue.', false);
            updateSummary();
        }
    }
    ...
    function updateSummary() {
        ...
        const summaryRegion = document.getElementById('summaryPlayerRegion');
        const regionSelect = document.getElementById('playerRegion');

        const rawUid = uidInput ? uidInput.value.trim() : '';
        const region = regionSelect ? regionSelect.value : (state.verifiedInfo.region || 'IND');

        if (summaryUid) {
            summaryUid.innerText = (state.verifiedInfo.verified && state.verifiedInfo.uid)
                ? state.verifiedInfo.uid
                : (rawUid ? `${rawUid} (Unverified)` : 'Not Verified');
        }
        if (summaryName) {
            summaryName.innerText = (state.verifiedInfo.verified && state.verifiedInfo.nickname)
                ? state.verifiedInfo.nickname
                : '--';
        }
        if (summaryRegion) {
            summaryRegion.innerText = state.verifiedInfo.region || region;
        }
    }
    ...
    function processPaymentModal() {
        ...
        if (!rawUid || !state.verifiedInfo.verified) {
            showToast('Please enter and verify your Player UID first!', true);
        ...
        if (modalUid) {
            modalUid.innerText = state.verifiedInfo.uid;
        }
        if (modalNickname) {
            modalNickname.innerText = state.verifiedInfo.nickname;
        }
        if (modalRegion) {
            modalRegion.innerText = state.verifiedInfo.region || 'IND';
        }
        if (modalAmount) {
    ...
    function handleReceiptSubmit(e) {
        ...
        if (!rawUid || !state.verifiedInfo.verified) {
            showToast('Please enter and verify your Player UID!', true);
            return;
        }
        ...
        const verificationBadge = state.verifiedInfo.isManual
            ? `(Manual Entry - Verify in-game)`
            : `(Verified Live ✅)`;

        const waMsg = 
`*${CONFIG.STORE_NAME} - NEW ORDER*

*Player UID:* ${state.verifiedInfo.uid}
*In-Game Nickname:* ${state.verifiedInfo.nickname}
*Server Region:* ${state.verifiedInfo.region} ${verificationBadge}
